...
func TestExtended(t *testing.T) {
	...
	kf, est0, err := NewExtended(x0, Covar0, F, G, H, noise)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtendedMultiD(t *testing.T) {
	// DT system
	Δt := 0.01
	...
	kf, _, err := NewExtended(x0, Covar0, F, G, H, noise)
	t.Logf("%s", kf)
	if err != nil {
		panic(err)
	}
